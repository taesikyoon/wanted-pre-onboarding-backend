...
export class Init1696408303829 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`job-posting\` (\`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`create-at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated-at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`position\` INT NOT NULL, \`compensation\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`technical-stack\` varchar(255) NOT NULL, \`companyId\` bigint UNSIGNED NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
